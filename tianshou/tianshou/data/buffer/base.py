...
class ReplayBuffer:
    ...
    def sample(self, batch_size: int, all_seq=False) -> Tuple[Batch, np.ndarray]:
        ...
        return self.get_data(indices, all_seq), indices
    ...
    def get(
        ...
        if key not in self._meta and from_val is not None:
          val = from_val
        else:
          val = self._meta[key]
    ...
    def get_data(self, index: Union[slice, int, List[int], np.ndarray], all_seq=False) -> Batch:
        ...
        if all_seq:
          return Batch(
              obs=obs,
              act=self.get(indices, 'act'),
              rew=self.get(indices, 'rew'),
              done=self.get(indices, 'done'),
              obs_next=obs_next,
