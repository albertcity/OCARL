...
class Checkpointer:
    ...
    def load_last(self, path, model, optimizer_fg, optimizer_bg, use_cpu=False, return_iter=False):
        """
        If path is '', we load the last checkpoint
        """
        print(path, self.listfile)            
        ...
        if path == '':
            with open(self.listfile, 'rb') as f:
                ...
                if len(model_list) == 0:
                    ...
                    path = model_list[-1]
        path = osp.join(*self.listfile.split('/')[0:-1], path.split('/')[-1])
        if self.listfile[0] == '/':
          path = '/' + path
        if return_iter:
          iter_n = int(path.split('_')[-1].split('.')[0])
        print(f'Load last model from {self.listfile}, {path}')
        if return_iter:
          return iter_n, self.load(path, model, optimizer_fg, optimizer_bg, use_cpu)
        else:
