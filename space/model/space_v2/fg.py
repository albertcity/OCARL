...
class ZWhatEnc(nn.Module):
    def __init__(self):
        ...
        glimpse_size = arch.glimpse_size   
        ...
        self.latent_dim = self.enc_cnn(torch.ones(1, 3, glimpse_size, glimpse_size)).size()
        self.enc_what = nn.Linear(latent_dim, arch.z_what_dim * 2)
    
    def forward(self, x):
        """
        Encode a (12, 12) glimpse into z_what
...
class GlimpseDec(nn.Module):
    ...
    def __init__(self, first_shape=[64,3,3]):
        ...
        self.dec = nn.Sequential(
          ...
          # nn.ConvTranspose2d(32, 32, 2, stride=2, bias=True), nn.ReLU(inplace=True),  # (32, 4, 4)
          # nn.Conv2d(32,16,3, padding=1, bias=True), nn.ReLU(inplace=True), # (16, 4,4)
          # nn.ConvTranspose2d(16, 16, 2, stride=2, bias=True), nn.ReLU(inplace=True),  # (16, 8, 8)
          # nn.Conv2d(16,image_shape[-1] + 1, 3, padding=1, bias=True), #(3, 8, 8)
          # nn.ConvTranspose2d(first_shape[0], ),
        # self.dec = nn.Sequential(
