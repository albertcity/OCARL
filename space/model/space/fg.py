...
class SpaceFg(nn.Module):
    ...
    def forward(self, x, globel_step, encode_only=False):
        """
        Forward pass
        ...
        assert ((kl_z_pres.size() == (B, self.arch.G ** 2, 1)) and
                ...
                (kl_z_shift.size() == (B, self.arch.G ** 2, 2)) and
                (kl_z_what.size() == (B, self.arch.G ** 2, self.arch.z_what_dim))
                )
...
class ImgEncoderFg(nn.Module):
    ...
    def __init__(self, arch):
        ...
        self.enc = nn.Sequential(
            ...
            nn.CELU(),
            nn.GroupNorm(8, 64),
            nn.Conv2d(64, 64, 3, second_to_last_stride, 1),
            nn.CELU(),
            nn.GroupNorm(16, 64),
            nn.Conv2d(64, 128, 3, last_stride, 1),
            nn.CELU(),
            nn.GroupNorm(32, 128),
            nn.Conv2d(128, self.arch.img_enc_dim_fg, 1),
            nn.CELU(),
        ...
        self.enc_cat = nn.Sequential(
            nn.Conv2d(self.arch.img_enc_dim_fg * 2, 64, 3, 1, 1),
            nn.CELU(),
            nn.GroupNorm(16, 64)
        )
        
        # Image encoding -> latent distribution parameters (B, 128, G, G) -> (B, D, G, G)
        self.z_scale_net = nn.Conv2d(64, (self.arch.z_where_scale_dim) * 2, 1)
        self.z_shift_net = nn.Conv2d(64, (self.arch.z_where_shift_dim) * 2, 1)
        self.z_pres_net = nn.Conv2d(64, self.arch.z_pres_dim, 1)
        self.z_depth_net = nn.Conv2d(64, self.arch.z_depth_dim * 2, 1)
...
class ZWhatEnc(nn.Module):
    ...
    def __init__(self, arch):
        ...
        self.enc_what = nn.Linear(128, self.arch.z_what_dim * 2)
    ...
    def __init__(self, arch):
        ...
        if arch.glimpse_size == 32:
          self.dec = nn.Sequential(
              nn.Conv2d(self.arch.z_what_dim, 128, 1),
              nn.CELU(),
              nn.GroupNorm(16, 128),
