...
class Space(nn.Module):
    ...
    def __init__(self, arch):
        ...
        self.arch = arch
        self.fg_module = SpaceFg(arch)
        self.bg_module = SpaceBg(arch)
    ...
    def forward(self, x, global_step):
        ...
        # Fix alpha trick
        if global_step and global_step < self.arch.fix_alpha_steps:
            alpha_map = torch.full_like(alpha_map, self.arch.fix_alpha_value)
        ...
        loss = loss + log_fg.get('sep_total_loss', 0)
        log = {
            'imgs': x,
            'y': y,
            # (B,)
