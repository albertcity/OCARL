...
class SpaceBg(nn.Module):
    ...
    def __init__(self, arch):
        ...
        self.arch = arch
        self.image_enc = ImageEncoderBg(arch)
        ...
        self.predict_mask = PredictMask(arch)
        # Compute masks given mask latents
        self.mask_decoder = MaskDecoder(arch)
        # Encode mask and image into component latents
        self.comp_encoder = CompEncoder(arch)
        # Component decoder

        # self.comp_decoder = CompDecoder()
        if arch.K > 1:
            self.comp_decoder = CompDecoder(arch)
        else:
            self.comp_decoder = CompDecoderStrong(arch)
        ...
        self.predict_mask_prior = PredictMask(arch)
        # Compute component latents
        self.predict_comp_prior = PredictComp(arch)
        # ==== Prior related ====
    ...
    def forward(self, x, global_step):
        ...
        z_mask = self.z_mask_0.expand(B, self.arch.z_mask_dim)
        h = self.rnn_mask_h.expand(B, self.arch.rnn_mask_hidden_dim)
        c = self.rnn_mask_c.expand(B, self.arch.rnn_mask_hidden_dim)
        
        for i in range(self.arch.K):
        ...
        h = self.rnn_mask_h_prior.expand(B, self.arch.rnn_mask_prior_hidden_dim)
        c = self.rnn_mask_c_prior.expand(B, self.arch.rnn_mask_prior_hidden_dim)
...
class ImageEncoderBg(nn.Module):
    ...
    def __init__(self, arch):
        self.arch = arch
        embed_size = self.arch.img_shape[0] // 16
        nn.Module.__init__(self)
        self.enc = nn.Sequential(
            ...
            nn.ELU(),
            Flatten(),
            nn.Linear(64 * embed_size ** 2, self.arch.img_enc_dim_bg),
            nn.ELU(),
        )
    ...
    def forward(self, x):
        ...
        """
        out = self.enc(x)
        if self.arch.get('no_bg_enc', False):
          out = out * 0
        return out
...
class PredictMask(nn.Module):
    ...
    def __init__(self, arch):
        self.arch = arch
        nn.Module.__init__(self)
        self.fc = nn.Linear(self.arch.rnn_mask_hidden_dim, self.arch.z_mask_dim * 2)
    ...
    def forward(self, h):
        ...
        """
        x = self.fc(h)
        z_mask_loc = x[:, :self.arch.z_mask_dim]
        z_mask_scale = F.softplus(x[:, self.arch.z_mask_dim:]) + 1e-4
...
class MaskDecoder(nn.Module):
    ...
    def __init__(self, arch):
        ...
        self.dec = nn.Sequential(
            nn.Conv2d(self.arch.z_mask_dim, 256, 1),
            nn.CELU(),
            nn.GroupNorm(16, 256),
            ...
            layers if arch.img_shape[0] == 128 else nn.Conv2d(256, 128, 3, 1, 1),
            # nn.Conv2d(256, 128 * 2 * 2, 1),
            # nn.PixelShuffle(2),
            # nn.CELU(),
            # nn.GroupNorm(16, 128),
...
class CompEncoder(nn.Module):
    ...
    def __init__(self, arch):
        ...
        self.enc = nn.Sequential(
            ...
            nn.Linear(64 * embed_size ** 2, self.arch.z_comp_dim * 2),
        )
    ...
    def forward(self, x):
        ...
        if self.arch.get('no_bg_enc', False):
          x = x * 0
        z_comp_loc = x[:, :self.arch.z_comp_dim]
        z_comp_scale = F.softplus(x[:, self.arch.z_comp_dim:]) + 1e-4
...
class CompDecoder(nn.Module):
    ...
    def __init__(self, arch):
        ...
        self.decoder = nn.Sequential(
            nn.Conv2d(self.arch.z_comp_dim + 2, 32, 3, 1),
            nn.BatchNorm2d(32),
            nn.ELU(),
            nn.Conv2d(32, 32, 3, 1),
            nn.BatchNorm2d(32),
...
class CompDecoderStrong(nn.Module):
    ...
    def __init__(self, arch):
        ...
        self.dec = nn.Sequential(
            nn.Conv2d(self.arch.z_comp_dim, 256, 1),
            nn.CELU(),
            nn.GroupNorm(16, 256),
            ...
            layers if arch.img_shape[0] == 128 else nn.Conv2d(128, 64, 3, 1, 1),
            # nn.Conv2d(128, 64 * 2 * 2, 1),
            # nn.PixelShuffle(2),
            # nn.CELU(),
            # nn.GroupNorm(8, 64),
...
class PredictComp(nn.Module):
    ...
    def __init__(self, arch):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(self.arch.z_mask_dim, self.arch.predict_comp_hidden_dim),
            nn.ELU(),
            nn.Linear(self.arch.predict_comp_hidden_dim, self.arch.predict_comp_hidden_dim),
            nn.ELU(),
            nn.Linear(self.arch.predict_comp_hidden_dim, self.arch.z_comp_dim * 2),
        )
