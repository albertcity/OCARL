...
class SpaceFg(nn.Module):
    ...
    def forward(self, x, globel_step):
        ...
        # (B*G*G, D)
        z_what, z_what_e, z_what_q = self.z_what_net(x_att)
        ...
        z_what_e = z_what_e.view(B, arch.G**2, arch.z_what_dim)
        z_what_q = z_what_q.view(B, arch.G**2, arch.z_what_dim)
        # z_what = z_what.view(B, arch.G ** 2, arch.z_what_dim)
        ...
        beta = 1.
        kl_z_what = beta * F.mse_loss(z_what_e, z_what_q.detach(), reduction='none') + F.mse_loss(z_what_e.detach(), z_what_q, reduction='none')
...
class ZWhatEnc(nn.Module):
    ...
    def __init__(self):
        ...
        self.enc_what = nn.Linear(256, arch.z_what_dim)
        self.vq_code = VQEmbedding(arch.vq_K, arch.z_what_dim) 
    ...
    def forward(self, x):
        ...
        """
        x_e = self.enc_cnn(x)
        x_e = self.enc_what(x_e.flatten(start_dim=1))

        x_st, x_q = self.vq_code.straight_through(x_e)
        return x_st, x_e, x_q
